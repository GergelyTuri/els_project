...
def calculate_age_at_sefla(data):
    ...
    for idx, row in data.iterrows():
        if row["day"] == "sefla":
            ...
            sefla_row = data[
                (data["cohort_id"] == row["cohort_id"]) & (data["day"] == "sefla")
            ]
...
class AnalysisTools:
    ...
    def repeated_measures_anova(
        ...
        self.data = self.data[
            (self.data["day"] != "sefla") & (self.data["day"] != "recall5")
        ]
        ...
        eta_squared = anova_result["n2"].iloc[0]
        ...
        power = power_anova(eta_squared, k=2, n=self.data["cohort_id"].nunique())
    ...
    def post_hoc_analysis_bouts_significant(
        self, dv, within, between, subject, p_adjust="bonferroni", alpha=0.05
    ):
        """
        Perform post hoc pairwise comparisons and display only significant results.
        ...
        # Select relevant columns
        significant_results = significant_results[
            ["Contrast", "minute_bin", "A", "B", "p-corr", "cohen"]
        ]
